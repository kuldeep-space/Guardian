...
class GuardianAccessibilityService : AccessibilityService() {
    ...
    private val screenReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            if (intent?.action == Intent.ACTION_SCREEN_OFF) {
                android.util.Log.d("GuardianAI_Debug", "[AS] SCREEN_OFF received. Resetting whitelist.")
                resetWhitelist()
            }
        }
    }
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        if (type == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED ||
            ...
            // Check the foreground package when events trigger
    ...
    private fun startPeriodicVerification() {
        ...
        verificationJob = serviceScope.launch {
            ...
            while (isActive) {
                delay(400)
                // Log every 10th iteration (~4s) to prove the loop is alive without flooding logcat
                if (System.currentTimeMillis() % 4000 < 400) {
    ...
    private fun handlePackageChangeCheck() {
        ...
        if (detectedPackage.isEmpty()) return
